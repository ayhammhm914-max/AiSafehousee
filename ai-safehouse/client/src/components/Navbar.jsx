...
function SafehouseLogoMark() {
  return (
    <img
      src="/ai-safehouse-logo.svg"
    ...
    <nav className="relative z-30 px-4 pt-4 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-[2rem] border border-white/10 bg-slate-950/22 px-4 py-3 shadow-[0_18px_60px_rgba(2,6,23,0.45)] backdrop-blur-2xl sm:px-5 lg:px-6">
        <TransitionNavButton to="/" tone="cyan" className="group flex min-w-0 items-center gap-3 rounded-2xl pr-2 text-left transition">
          <div className="logo-signal relative flex h-16 w-16 items-center justify-center rounded-2xl border border-cyan-300/15 bg-[linear-gradient(145deg,rgba(103,232,249,0.12),rgba(15,23,42,0.75),rgba(167,139,250,0.18))] p-2.5 shadow-[0_0_35px_rgba(34,211,238,0.14)] sm:h-20 sm:w-20">
            <SafehouseLogoMark />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.45em] text-cyan-300/75 sm:text-[11px]">AI Safehouse</p>
            <h1 className="bg-gradient-to-r from-cyan-200 via-sky-300 to-violet-300 bg-clip-text text-lg font-black text-transparent sm:text-2xl">Ai SafeHouse</h1>
            <p className="hidden text-xs text-slate-300/70 sm:block">Discover tools, ask AiMaster, and navigate the full AI vault.</p>
          </div>
        </TransitionNavButton>
